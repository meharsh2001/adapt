...
server.use(function (err, req, res, next) {
  logger.log('error', `${err.status || 500} ${JSON.stringify(user._id)} ${req.sessionID} ||: ${res.statusMessage} : ${err.message} : ${req.originalUrl} : ${req.method} : ${req.ip}`);
  next()
})
server.use(expressWinston.logger({
  transports: [new winston.transports.DailyRotateFile({
    filename: `logs/all-%DATE%.log`, datePattern: "DD-MM-YYYY", maxFiles: "1d", format: format.combine(format.timestamp({ format: 'MMM-DD-YYYY HH:mm:ss' }),
      format.align(),
...
server.use(function (req, res, next) {
  ...
  res.on("finish", function () {
    ...
    if (res.statusCode > 400) {
      logger.log('error', `USER:${user} SESSION ID:${req.sessionID} || ${res.statusCode}:${req.method}:${res.statusMessage} :IP${req.ip} || ${req.originalUrl}: ${responsetime}ms`);
