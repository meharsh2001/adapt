...
// recording 500 error calls
server.use(function(err,req,res,next){
  logger.log('error',`${err.status || 404} ${JSON.stringify(user._id)} ${req.sessionID} ||: ${res.statusMessage} : ${err.message} : ${req.originalUrl} : ${req.method} : ${req.ip}`);
  next()
})
server.use(expressWinston.logger({
  transports: [new winston.transports.DailyRotateFile({filename: `logs/all-%DATE%.log`, datePattern: "DD-MM-YYYY", maxFiles: "1d",format:format.combine(format.timestamp({format: 'MMM-DD-YYYY HH:mm:ss'}),
  format.align(),
 format.printf(info => `${info.level}: ${[info.timestamp]}: ${info.message}`),
 )})],
}));
//recording rest all calls
server.use(function(req,res,next){ 
  ...
  res.on("finish", function() {
    ...
    try{ var user = JSON.stringify(user._id);}
    catch(err){ user = 'not loggedin' ; }   
/*      try{
  ...
  if(req.xhr == true ){
    logger.log('info',`USER:${user} SESSION ID:${req.sessionID} || ${res.statusCode}:${req.method}:${res.statusMessage} :IP${req.ip} || ${req.originalUrl}: ${responsetime}ms`);
  }else if(res.statusCode>400){
    logger.log('warn',`USER:${user || 404} SESSION ID:${req.sessionID} || ${res.statusCode}:${req.method}:${res.statusMessage} :IP${req.ip} || ${req.originalUrl}: ${responsetime}ms`);
  }
})
next()
});
