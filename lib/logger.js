...
function Log() {
  this.logger = winston.createLogger({
    exitOnError: false,
    transports: [new winston.transports.DailyRotateFile({filename: `logs/server-%DATE%.log`, datePattern: "DD-MM-YYYY", maxFiles: "1d",format:format.combine(format.timestamp({format: 'MMM-DD-YYYY HH:mm:ss'}),
    format.align(),
   format.printf(info => `${info.level}: ${[info.timestamp]}: ${info.message}`),
   )}),new winston.transports.Console()],
    format: winston.format.printf((info) => {
      const colorise = chalk[colours[info.level]] || (level => level);
      return `${colorise(info.level)} [${chalk.green(moment().format('DD MMM YY HH:mm:ss Z'))}] ${colorise(info.message)}`;
    })
  });
}
/**
